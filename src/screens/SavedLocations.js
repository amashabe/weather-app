...
import { StyleSheet, View, Dimensions, TouchableOpacity, ActivityIndicator, Image } from "react-native";
...
export default function SavedLocations({ navigation }) {
  ...
  const fetchWeatherData = async (latitude, longitude) => {
    ...
    const response = await fetch(`${currentWeatherApiUrl}?lat=${latitude}&lon=${longitude}&appid=${EXPO_PUBLIC_OPEN_WEATHER_API_KEY}&units=metric`);
    ...
    const weatherData = { general, description, currentTemperature, };
    return weatherData;
  };
  ...
  const fetchWeatherForAllLocations = async (locations) => {
    ...
    try {
      ...
      return weatherData.map((data, index) => ({ ...locations[index], ...data, }));
    } catch (error) {
  ...
  const fitAllMarkersOnTheMap = (data) => {
    if (mapViewRef.current) {
      ...
      mapViewRef.current.fitToSuppliedMarkers(markersRefs, { edgePadding: { top: DEVICE_HEIGHT * 0.15, right: DEVICE_HEIGHT * 0.08, bottom: DEVICE_HEIGHT * 0.35, left: DEVICE_HEIGHT * 0.08, }, });
    }
  };

  const CustomPin = () => (
    <View style={styles.locationContainer}>
      <Image style={[styles.pinImage]} source={require("../../assets/Images/saved-location.png")} />
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      {isLoadingSavedLocations ? (<ActivityIndicator size={"large"} color={"grey"} style={{ marginTop: DEVICE_HEIGHT / 2 }} />) : (
        <>
          <MapView provider={MapView.PROVIDER_GOOGLE} ref={mapViewRef} style={styles.map}>
            {savedLocationsWeatherData.map(
              ({
                ...
                <Marker coordinate={{ latitude, longitude, }} title={placeName} description={`${currentTemperature}°C, ${description}`} identifier={placeName} key={placeName}>
                  <CustomPin />
                </Marker>
              )
            )}
