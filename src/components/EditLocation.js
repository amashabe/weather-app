...
const { EXPO_PUBLIC_MAPBOX_ACCESS_TOKEN, EXPO_PUBLIC_OPEN_WEATHER_API_KEY } =
  process.env;
...
export default function EditLocation({
  ...
  const fetchWeatherData = async (latitude, longitude) => {
    ...
    const response = await fetch(
      `${currentWeatherApiUrl}?lat=${latitude}&lon=${longitude}&appid=${EXPO_PUBLIC_OPEN_WEATHER_API_KEY}&units=metric`
    );
  ...
  const searchPlaces = (query) => {
    ...
    fetch(
      `${geocodingEndpoint}?access_token=${EXPO_PUBLIC_MAPBOX_ACCESS_TOKEN}`
    )
